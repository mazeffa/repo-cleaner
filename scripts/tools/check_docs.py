...
and in output):
  ...
  research-fields       Research docs carry Status:/Date:, and a superseded one names
                         what replaced it.
...
def check_decisions_integrity(ctx, errors):
    ...
    for e in entries:
        ...
        ids[eid] = e
        # Fields live on the lines below the heading. Searching the heading too means a
        # decision titled e.g. "... carry Status:/Date:" parses its own title as a field.
        body = e.split("\n", 1)[1] if "\n" in e else ""
...
def check_research_fields(ctx, errors):
    ...
    if not d.exists():
        return
    status_re = re.compile(FIELD_RE_TMPL.format("Status"), re.M)
    date_re = re.compile(FIELD_RE_TMPL.format("Date"), re.M)
    sup_re = re.compile(FIELD_RE_TMPL.format("Superseded-by"), re.M)
    for p in sorted(d.glob("*.md")):
        ...
        if not dm or not DATE_RE.match(dm.group(1).strip()):
            errors.append(f"{rel}: bad/missing Date (YYYY-MM-DD)")
        if status == "superseded" and not (sup_re.search(text) and sup_re.search(text).group(1).strip()):
...
def run_generic_rules(ctx, errors):
    ...
    check_research_fields(ctx, errors)
...
def _selfcheck():
    ...
    assert any("research/bad.md: bad/missing Status" in e.replace("\\", "/") for e in found), found
    assert any("research/bad.md: bad/missing Date" in e.replace("\\", "/") for e in found), found
    assert any("research/orphan.md: Status: superseded but names no" in e.replace("\\", "/") for e in found), found
    assert not any("research/good.md" in e.replace("\\", "/") for e in found), found
